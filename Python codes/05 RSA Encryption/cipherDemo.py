...
def decryptor(encryptedMessage,privateKey):        #Function for decrypting the encryptedMessage
    originalMessage = pow(encryptedMesasge,privateKey[1]) % privateKey[0]
...
encryptedMesasge = encryptor(userInput,publicKey)
print("\nThe Encypted Message is  : "+str(encryptedMesasge))

originalMessage = decryptor(encryptedMesasge,privateKey)
print("\nThe Decrypted Message is : "+str(originalMessage))
